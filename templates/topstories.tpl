...
{if $story.pubdate}
            <td class="text-right text-muted hint" title="Published {$story.pubdate|date_format:"%A, %B %e, %Y"} at {$story.pubdate|date_format:"%l:%M %p, %Z"}"><span class="visible-xs-inline">{$story.pubdate|date_format:"%a"}</span><span class="hidden-xs">{$story.pubdate|date_format:"l"} <span style='color:silver; padding-left:2em'>{$story.pubdate|ago} ago</span></span></td>
{else}
